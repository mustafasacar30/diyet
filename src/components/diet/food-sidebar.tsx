...
export function FoodEditDialog({
    ...
    const autoMatchedCard = useMemo(() => {
        ...
        const candidates = cards.map(c => {
            ...
            if (foodBans.some(b => b.card_filename === c.filename)) return { card: c, score: 0 }
    ...
    const activeThumbnail = useMemo(() => {
        ...
        return autoMatchedCard ? autoMatchedCard.url : null
    }, [name, cards, foodMatches, autoMatchedCard])
                                    ...
                                    {autoMatchedCard && !foodMatches.some(m => m.card_filename === autoMatchedCard.filename) && (
                                        <div className="mb-2">
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-0.5">
                                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-blue-50 text-blue-800 border border-blue-200">
                                                    ✨ {autoMatchedCard.filename.replace(/\..+$/, '')}
                                                    <button onClick={(e) => { e.preventDefault(); addBan(foodPattern, autoMatchedCard.filename) }} className="hover:text-red-600 ml-0.5" title="Bu otomatik eşleşmeyi yasakla">
                                                        <X size={10} />
                                                    </button>
                                                </span>
                                            </div>
